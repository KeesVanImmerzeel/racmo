...
#' df <- get_filenames(url, maxKeys = 500, sorting = "desc", token = token)
#' head(df)
#' }
#'
#' @export
get_filenames <- function(url, maxKeys = 100000, sorting = "asc", token) {
  ...
  response <- httr::GET(
    url,
    httr::add_headers(Authorization = base::paste("Bearer", token)),
    query = params
  )
  ...
  if (httr::status_code(response) == 200) {
    ...
    df <- base::as.data.frame(content_list$files)
    return(df)
  } else {
...
download_and_load_raster <- function(filename, varid = "precip", token, url) {
  # Clean tempdir
  files <- base::list.files(base::tempdir(), full.names = TRUE)  # base
  base::unlink(files, recursive = TRUE)                          # base

  # Build file URL
  file_url <- base::paste0(url, "/", filename, "/url")           # base

  # Get download URL
  file_response <- httr::GET(                                    # httr
    file_url,
    httr::add_headers(Authorization = base::paste("Bearer", token))  # httr, base
  )
  if (httr::status_code(file_response) != 200)                  # httr
    base::stop("Failed to get file URL for ", filename)          # base
  ...
  local_path <- base::file.path(base::tempdir(), filename)       # base
  ...
  rlon <- base::range(ncdf4::ncvar_get(nc, varid = "lon"))      # ncdf4, base
  rlat <- base::range(ncdf4::ncvar_get(nc, varid = "lat"))      # ncdf4, base
  ext <- terra::ext(base::c(rlon, rlat))                        # terra, base
  ...
  cf <- CFtime::CFtime(                                          # CFtime
    units = time_units,
    calendar = ifelse(base::is.null(time_calendar), "gregorian", time_calendar),
    vals = time_vals
  )
  dates <- base::as.Date(CFtime::as_timestamp(cf))              # CFtime, base
  date_strings <- base::format(dates, "%Y-%m-%d")              # base
  names(rstack) <- base::paste0(varid, "_", date_strings) # terra, base
